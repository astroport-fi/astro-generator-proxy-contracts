...
use astroport_generator_proxy::mars_staking::{
    Cw20HookMsg as MarsCw20HookMsg, ExecuteMsg as MarsExecuteMsg,
};
...
fn test_proper_initialization() {
    ...
    let msg = InstantiateMsg {
        ...
        lp_token_addr: "marsust0000".to_string(),
        reward_contract_addr: "reward0000".to_string(),
        reward_token_addr: "mars0000".to_string(),
    };
    ...
    assert_eq!("marsust0000", config.lp_token_addr.as_str());
    assert_eq!("reward0000", config.reward_contract_addr.as_str());
    assert_eq!("mars0000", config.reward_token_addr.as_str());
}
...
fn test_deposit() {
    ...
    let info = mock_info("marsust0000", &[]);
    let deposit_msg = ExecuteMsg::Receive(Cw20ReceiveMsg {
    ...
    assert_eq!(
        ...
        vec![SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: "marsust0000".to_string(),
            funds: vec![],
            msg: to_binary(&Cw20ExecuteMsg::Send {
                contract: "reward0000".to_string(),
                amount: Uint128::from(100u128),
                msg: to_binary(&MarsCw20HookMsg::Bond {}).unwrap(),
            })
            .unwrap(),
        }))]
    );
...
fn test_update_rewards() {
    ...
    assert_eq!(
        ...
        vec![SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            funds: vec![],
            msg: to_binary(&MarsExecuteMsg::Claim {}).unwrap(),
        }))]
    );
...
fn test_send_rewards() {
    ...
    assert_eq!(
        res.messages,
        vec![SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: "mars0000".to_string(),
...
fn test_query_reward_info() {
    ...
    assert_eq!(query_res, Addr::unchecked("mars0000"));
}
