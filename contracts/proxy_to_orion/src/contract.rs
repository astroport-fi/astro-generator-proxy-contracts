...
use astroport_generator_proxy::orion_staking::{
    Cw20HookMsg as OrionCw20HookMsg, ExecuteMsg as OrionExecuteMsg, QueryMsg as OrionQueryMsg,
    StakerInfoResponse,
};
use cw2::set_contract_version;

// version info for migration info
const CONTRACT_NAME: &str = "astroport-generator-proxy-to-orion";
...
/// Stakes them with the ORION LP Staking contract
fn receive_cw20(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
...
/// @dev Claims pending rewards from the ORION LP staking contract
fn update_rewards(deps: DepsMut) -> Result<Response, ContractError> {
    ...
    response
        ...
        .push(SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            funds: vec![],
            msg: to_binary(&OrionExecuteMsg::Claim {})?,
        })));

    Ok(response)
}

/// @dev Transfers ORION rewards
/// @param account : User to which ORION tokens are to be transferred
/// @param amount : Number of ORION to be transferred
fn send_rewards(
    deps: DepsMut,
    info: MessageInfo,
    account: Addr,
...
fn withdraw(
    ...
    response.messages.push(SubMsg::new(WasmMsg::Execute {
        contract_addr: cfg.reward_contract_addr.to_string(),
        funds: vec![],
        msg: to_binary(&OrionExecuteMsg::Unbond { amount })?,
    }));
