...
use astroport_generator_proxy::generator_proxy_apollo::{
    CallbackMsg, Cw20HookMsg, ExecuteMsg, InstantiateMsg, QueryMsg,
};
...
fn test_proper_initialization() {
    ...
    let msg = InstantiateMsg {
        ...
        lp_token_addr: "apolloust0000".to_string(),
        reward_contract_addr: "reward0000".to_string(),
        reward_token_addr: "apollo0000".to_string(),
        strategy_id: 3,
    };
    ...
    assert_eq!("apolloust0000", config.lp_token_addr.as_str());
    assert_eq!("reward0000", config.reward_contract_addr.as_str());
    assert_eq!("apollo0000", config.reward_token_addr.as_str());
    assert_eq!(3, config.strategy_id);
}
...
fn test_deposit() {
    ...
    let info = mock_info("apolloust0000", &[]);
    let deposit_msg = ExecuteMsg::Receive(Cw20ReceiveMsg {
    ...
    assert_eq!(
        ...
        vec![SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: "apolloust0000".to_string(),
            funds: vec![],
            msg: to_binary(&Cw20ExecuteMsg::Send {
                contract: "reward0000".to_string(),
                amount: Uint128::from(100u128),
                msg: to_binary(&ApolloFacCw20HookMsg::Deposit { strategy_id: 3 }).unwrap(),
            })
            .unwrap(),
        }))]
    );
...
fn test_send_rewards() {
    ...
    assert_eq!(
        res.messages,
        vec![SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: "apollo0000".to_string(),
...
fn test_query_reward_info() {
    ...
    assert_eq!(query_res, Addr::unchecked("apollo0000"));
}
