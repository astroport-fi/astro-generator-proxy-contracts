...
/// @dev Receives LP tokens sent by Generator contract. Further sends them to the Whale LP Staking contract
fn receive_cw20(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
        ...
        response
            ...
            .push(SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                msg: to_binary(&Cw20ExecuteMsg::Send {
                    ...
                    msg: to_binary(&WhaleCw20HookMsg::Bond {})?,
                })?,
            })));
    } else {
...
/// @dev Claims pending rewards from the Whale LP staking contract
fn update_rewards(deps: DepsMut) -> Result<Response, ContractError> {
    ...
    response
        ...
        .push(SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            funds: vec![],
            msg: to_binary(&WhaleExecuteMsg::Claim {})?,
        })));

    Ok(response)
}

/// @dev Transfers WHALE rewards
/// @param account : User to which WHALE tokens are to be transferred
/// @param amount : Number of WHALE to be transferred
fn send_rewards(
    deps: DepsMut,
    info: MessageInfo,
    account: Addr,
